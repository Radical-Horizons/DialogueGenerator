...
**Règles complémentaires (lire si besoin)** : `.cursor/rules/frontend.mdc`, `.cursor/rules/responsive_frontend.mdc`, `.cursor/rules/frontend_testing.mdc`, `.cursor/rules/graph_editor.mdc`, `.cursor/rules/workflow.mdc`
...
| Auth dev | Ne pas durcir `DISABLE_AUTH` / mock admin sans demande explicite (voir `AGENTS.md`) |
...
7. **Preuve UI** : `npm run dev` — ≥320px, ~480px ou colonne étroite (voir `workflow.mdc`).
...
Hooks DOM : voir `frontend_testing.mdc` (callback ref 17.8, `style.width` en RTL).
