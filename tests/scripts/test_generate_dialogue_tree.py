...
from scripts.generate_dialogue_tree import format_path, pick_path, pick_random_path
...
def test_pick_random_path_is_reproducible_with_seed() -> None:
    nodes = [
        {
            "id": "START",
            "speaker": "PNJ",
    ...
    path_a, choices_a = pick_random_path(nodes, rng=random.Random(42))
    path_b, choices_b = pick_random_path(nodes, rng=random.Random(42))
    assert choices_a == choices_b
    assert [n["id"] for n in path_a] == [n["id"] for n in path_b]
