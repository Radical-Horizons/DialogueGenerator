...
class TestLLMClientFactory:
    ...
    def test_create_client_model_not_found(self):
        ...
        config = {"api_key_env_var": "OPENAI_API_KEY"}
        available_models = [
            {"api_identifier": "openai/gpt-5.6-terra", "display_name": "GPT-5.2"}
        ]
    ...
    def test_create_client_no_api_key_env_var(self):
        ...
        available_models = [
            {"api_identifier": "openai/gpt-5.6-terra", "display_name": "GPT-5.2", "client_type": "openai"}
        ]
    ...
    def test_create_openai_client_success(self, mock_openai_client_class):
        ...
        assert call_kwargs["config"]["default_model"] == "openai/gpt-5.6-luna"
    ...
    def test_create_client_uses_model_identifier_fallback(self):
        ...
        available_models = [
            {
                "model_identifier": "openai/gpt-5.6-luna",  # Pas d'api_identifier
                "display_name": "GPT-4o Mini",
                "client_type": "openai"
            }
        ]
    ...
    def test_create_openrouter_client_missing_key_falls_back_to_dummy(self):
        """Sans clé OpenRouter, la fabrique rend le Dummy comme les autres routes.

        Elle levait, ce qui était tenable tant qu'OpenRouter ne portait que
        quelques modèles. Depuis la bascule du catalogue entier le 2026-09-21,
        lever casserait la garantie du dépôt : développer sans clé LLM.

        Le repli n'est pas dangereux pour la mesure — `diagnose_models` détecte
        un `DummyLLMClient` rendu pour un modèle réel et déclare le candidat
        inutilisable, au lieu de noter des générations factices.
        """
