...
describe('generationConfigNormalization', () => {
  ...
  it('ne propose pas de correctif pour 60K + gpt-5.2', () => {
    const fixes = detectGenerationConfigFixes(
      { maxContextTokens: 300_000, maxCompletionTokens: 60_000, llmModel: MODEL_NAMES.GPT_5_2 },
      [{ model_identifier: MODEL_NAMES.GPT_5_2, display_name: 'GPT-5.2', client_type: 'openai', max_tokens: 128_000 }],
    )
    expect(fixes).toHaveLength(0)
  })

  it('ne propose pas de correctif pour gpt-5.4 (structured output Unity)', () => {
    const fixes = detectGenerationConfigFixes(
      { maxContextTokens: 300_000, maxCompletionTokens: 60_000, llmModel: MODEL_NAMES.GPT_5_4 },
      [{ model_identifier: MODEL_NAMES.GPT_5_4, display_name: 'GPT-5.4', client_type: 'openai', max_tokens: 128_000 }],
    )
    expect(fixes).toHaveLength(0)
  })
  ...
  it('propose un correctif modèle si Mistral sélectionné pour Unity', () => {
    const fixes = detectGenerationConfigFixes(
      ...
      [
        { model_identifier: 'gpt-5.2', display_name: 'GPT-5.2', client_type: 'openai', max_tokens: 128_000 },
