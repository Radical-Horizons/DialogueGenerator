...
- **T3** (suite E2E entière) : réservé au parent, à `npm run test:e2e:verify`, ou à la CI — **une** fois après les lots ciblés. Grille : `.claude/commands/test-tiers.md`.

## Anti-pattern (à refuser si tu es un enfant `Agent`)
...
Sous Claude Code ce rôle est un vrai subagent : `subagent_type: playwright-e2e-specialist` est disponible directement (contrairement à l'ancien harnais Cursor, où il fallait passer par des agents génériques).

Le parent envoie **plusieurs appels `Agent` dans le même message**, chacun avec :
