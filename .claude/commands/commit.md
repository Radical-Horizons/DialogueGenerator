---
description: Stage all, commit avec un message concis, push seulement si la gate CI T3 est verte.
argument-hint: "[message optionnel] [push]"
...
Contexte utilisateur : $ARGUMENTS
...
Avant tout `git push` vers `main`, exécuter la suite équivalente CI push main (voir `.claude/rules/ci_before_push.md`) :
...
2. `git add .` à la racine du dépôt — sauf périmètre explicite donné par l'utilisateur (« uniquement ces fichiers », liste de chemins). Détail : `.claude/rules/git_commit.md`.
