...
Source de vérité détaillée : **`.claude/rules/workflow.md`** (obligations agents, Vitest, Windows). Les skills BMAD *testarch* restent méthodo ; l'exécution concrète dans ce dépôt suit **cette grille** + `workflow.md`.
...
- **Windows** : ne pas pipe la sortie Vitest vers `Select-Object` (buffering) — redirection vers `tmp\vitest-out.txt` ou JSON (`test:ci`). Voir `workflow.md`.
- **Playwright en parallèle multi-agents** : `/playwright-e2e-parallel` — chaque enfant = chemins explicites, pas `npx playwright test` nu.
...
| `npm run test:all` | Version + pytest complet + script frontend (build + lint + unit) |

Implémentation backend : `scripts/pytest-tier.cjs` (évite les problèmes de quoting `-m` sous Windows).

## Prompts parent → enfants (E2E)

Découper par fichiers disjoints ; voir `/playwright-e2e-parallel`. Par défaut, un spécialiste E2E peut commencer par **`npm run test:e2e:smoke`** ou le lot T1 listé dans le ticket.
