...
description: 'Revue du diff courant (branche vs base) par les seuls reviewers concernés par les chemins touchés. Version ciblée de /full-review — à lancer avant d''ouvrir une PR.'
---
...
Si la branche cible n'est pas `main` (PR vers `dev`), utiliser `origin/dev...HEAD`.
...
## 2. Router vers les reviewers

Lancer **en parallèle, dans un seul tour**, un `Task` par reviewer dont au moins un chemin
correspond. Ne pas en lancer un huitième « généraliste » par-dessus.

| Chemins touchés | Reviewer |
|---|---|
...
| `e2e/`, `playwright*.config.ts` | `playwright-e2e-specialist` (lecture seule ici : signaler, ne pas réparer) |

`test-coverage-reviewer` se déclenche donc aussi sur un diff **sans** fichier de test — c'est
précisément le cas où son avis compte (`AGENTS.md` : test de régression obligatoire pour tout
fix de bug non trivial).
...
frontend run lint`, ou grep du motif — pas une lecture de code seule
(`AGENTS.md` : *require runtime log evidence before proposing bug fixes*).
