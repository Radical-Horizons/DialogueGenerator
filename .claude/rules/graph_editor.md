...
- **Validation inline à la sauvegarde** :
  - Échec PUT/save → `persistenceSlice.applySaveFieldValidationErrors` remplit `documentFieldErrors` (`uiSlice`) depuis `structured_errors` API.
  - Affichage : `InlineFieldError` + `fieldErrorBorder` sous le champ (`NodeEditorPanel`, `ChoiceEditor`) ; section liste → champ virtuel `choices`.
  - Premier nœud en erreur : auto-sélection + `graphViewStore.focusNode`. Toast = rappel court, pas le dump JSON.
  - Correction utilisateur → `clearDocumentFieldError(nodeId, field)` sur le champ édité. Mapper : `documentValidationFieldErrors.ts`. Règle API : `.claude/rules/api_validation_errors.md`.
