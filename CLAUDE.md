...
- **Templates et profils d'auteur** : deux objets serveur partageant une ACL unique (`services/owned_item_access.py`). Ne pas confondre `visibility` (statut persisté, `shared` par défaut) et `relation` (calculée pour le lecteur : `owned`/`team`/`legacy`). Invité = **lecture seule** (`.claude/rules/guest_first_auth.md`). Doc : `docs/guides/templates-et-profils-auteur.md` — sa section « Écarts connus » liste ce qui est livré mais faux, à commencer par un template partagé d'un collègue qui n'apparaît dans aucune liste.
