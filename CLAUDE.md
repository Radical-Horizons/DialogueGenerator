...
- **Templates et profils d'auteur** : **un seul objet réutilisable**. Presets et fiches livrées ont convergé (`services/template_convergence_service.py`, au démarrage, idempotent). Un template n'est qualifié que par son **statut** — `shared` (défaut) ou `private` — et `can_write == can_read` : qui voit peut modifier. Ne pas réintroduire de taxonomie (sections, provenances) : la visibilité est un statut, pas un découpage d'écran. Invité = lecture seule (`.claude/rules/guest_first_auth.md`). Doc : `docs/guides/templates-et-profils-auteur.md`.
